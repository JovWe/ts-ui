#!/bin/bash
# TS-UI: S-UI (sing-box) panel + TX-UI style installer
TSUI_FOLDER="${TSUI_FOLDER:-/usr/local/ts-ui}"
TSUI_SERVICE="${TSUI_SERVICE:-ts-ui}"

tsui_run_install() {
    local script_dir install_sh
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"
    install_sh="${script_dir}/install.sh"
    if [[ -f "$install_sh" ]]; then
        bash "$install_sh" "$@"
    elif [[ -n "${TSUI_INSTALL_URL}" ]]; then
        bash <(curl -Ls "${TSUI_INSTALL_URL}") "$@"
    else
        echo -e "${red}[ERR]${plain} install.sh not found. Set TSUI_INSTALL_URL or place install.sh beside ts-ui.sh"
        return 1
    fi
}
...
install() {
 bash <(curl -Ls TSUI_INSTALL_SCRIPT)
 if [[ $? == 0 ]]; then
 if [[ $# == 0 ]]; then
 start
 else
...
custom_version() {
 ...
 install_sh="${script_dir}/install.sh"
 if [[ -f "$install_sh" ]]; then
 bash "$install_sh" "$panel_version"
 elif [[ -n "${TSUI_INSTALL_URL}" ]]; then
 bash <(curl -Ls "${TSUI_INSTALL_URL}") "$panel_version"
 else
 LOGE "install.sh not found"
 exit 1
 fi
}
...
uninstall() {
 ...
 systemctl stop ts-ui
 systemctl disable ts-ui
 rm /etc/systemd/system/ts-ui.service -f
 systemctl daemon-reload
 systemctl reset-failed
 rm /etc/ts-ui/ -rf
 rm /usr/local/ts-ui/ -rf

 echo ""
 echo -e "Uninstalled Successfully, If you want to remove this script, then after exiting the script run ${green}rm /usr/local/ts-ui -f${plain} to delete it."
 echo ""
...
reset_admin() {
 ...
 if [[ $? == 0 ]]; then
 /usr/local/ts-ui/sui admin -reset
 fi
 before_show_menu
}
...
set_admin() {
 ...
 /usr/local/ts-ui/sui admin -username ${config_account} -password ${config_password}
 before_show_menu
}

view_admin() {
 /usr/local/ts-ui/sui admin -show
 before_show_menu
}
...
reset_setting() {
 ...
 if [[ $? == 0 ]]; then
 /usr/local/ts-ui/sui setting -reset
 fi
 before_show_menu
}
...
set_setting() {
 ...
 /usr/local/ts-ui/sui setting ${params}
 before_show_menu
}

view_setting() {
 /usr/local/ts-ui/sui setting -show
 view_uri
 before_show_menu
}

view_uri() {
 info=$(/usr/local/ts-ui/sui uri)
 if [[ $? != 0 ]]; then
...
update_shell() {
 wget -O /usr/bin/ts-ui -N --no-check-certificate "${TSUI_SCRIPT_RAW:-https://raw.githubusercontent.com/alireza0/s-ui/main/s-ui.sh}"
 if [[ $? != 0 ]]; then
 echo ""
 LOGE "Failed to download script, Please check whether the machine can connect Github"
 before_show_menu
 else
 chmod +x /usr/bin/ts-ui
...
show_ts_ui_status() {
 check_ts_ui_status
 if [[ $? == 0 ]]; then
 echo -e "ts-ui state: ${green}Running${plain}"
 else
 echo -e "ts-ui state: ${red}Not Running${plain}"
 fi
}
...
show_usage() {
 echo -e "TS-UI Control Menu Usage"
 echo -e "------------------------------------------"
 echo -e "SUBCOMMANDS:" 
 echo -e "ts-ui - Admin Management Script"
 echo -e "ts-ui start - Start ts-ui"
 echo -e "ts-ui stop - Stop ts-ui"
 echo -e "ts-ui restart - Restart ts-ui"
 echo -e "ts-ui status - Current Status of ts-ui"
 echo -e "ts-ui enable - Enable Autostart on OS Startup"
 echo -e "ts-ui disable - Disable Autostart on OS Startup"
 echo -e "ts-ui log - Check ts-ui Logs"
 echo -e "ts-ui update - Update"
 echo -e "ts-ui install - Install"
 echo -e "ts-ui uninstall - Uninstall"
 echo -e "ts-ui help - Control Menu Usage"
 echo -e "------------------------------------------"
}

show_menu() {
 echo -e "
 ${green}TS-UI Admin Management Script ${plain}
————————————————————————————————
 ${green}0.${plain} Exit
————————————————————————————————
...
————————————————————————————————
 ${green}11.${plain} TS-UI Start
 ${green}12.${plain} TS-UI Stop
 ${green}13.${plain} TS-UI Restart
 ${green}14.${plain} TS-UI Check State
 ${green}15.${plain} TS-UI Check Logs
 ${green}16.${plain} TS-UI Enable Autostart
 ${green}17.${plain} TS-UI Disable Autostart
————————————————————————————————
...
————————————————————————————————
 ...
 ;;
 11)
 check_install && start ts-ui
 ;;
 12)
 check_install && stop ts-ui
 ;;
 13)
 check_install && restart ts-ui
 ;;
 14)
 check_install && status ts-ui
 ;;
 15)
 check_install && show_log ts-ui
 ;;
 16)
 check_install && enable ts-ui
 ;;
 17)
 check_install && disable ts-ui
 ;;
 18)
 bbr_menu
 ;;
...
if [[ $# > 0 ]]; then
 case $1 in
 "start")
 check_install 0 && start ts-ui 0
 ;;
 "stop")
 check_install 0 && stop ts-ui 0
 ;;
 "restart")
 check_install 0 && restart ts-ui 0
 ;;
 "status")
 check_install 0 && status 0
 ;;
 "enable")
 check_install 0 && enable ts-ui 0
 ;;
 "disable")
 check_install 0 && disable ts-ui 0
 ;;
 "log")
 check_install 0 && show_log ts-ui 0
 ;;
 "update")
